...
print('вероятность выжить если пассажир -- представитель следующей категории: male and pclass:3:',
      df[(df['sex'] == 'male') & (df['pclass'] == 3)]['survived'].sum() / len(
...
x_train, x_test, y_train, y_test = train_test_split(df_only_numeric.drop('survived', axis=1),
                                                    df_only_numeric['survived'], random_state=random_state,
                                                    test_size=test_size, stratify=df_only_numeric['survived'])
...
print('Вычислите долю строк со значением Master относительно числа всех мужчин:',
      len(df_only_numeric[(df_only_numeric['honorific'] == 'Master') & (df['sex'] == 'male')]) / len(
          df_only_numeric[df['sex'] == 'male']))
...
dummies = pd.get_dummies(df.select_dtypes(exclude=np.number), prefix=df.select_dtypes(exclude=np.number).columns,
                         drop_first=True)
