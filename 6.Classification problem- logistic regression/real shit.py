...
dummies = pd.get_dummies(x_train.select_dtypes(exclude=np.number),
                         prefix=x_train.select_dtypes(exclude=np.number).columns, drop_first=True)
x_train[dummies.columns] = dummies
x_train.drop(x_train.select_dtypes(exclude=np.number).columns, axis=1, inplace=True)
dummies = pd.get_dummies(x_test.select_dtypes(exclude=np.number),
                         prefix=x_test.select_dtypes(exclude=np.number).columns, drop_first=True)
